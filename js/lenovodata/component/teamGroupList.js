;define('component/teamList', function(require, exports, module){
    ...
    function TeamGroupList(container, uid, data, delete_cb, combox_cb,combox_selectCallback) {
	...
	$.extend(TeamGroupList.prototype, {
        _init: function() {
            var self = this;
            var output = null;
            var title = null;
            ...
            for (var i=0, len=self.data.length; i<len; i++) {
                self.appendRoleItem(self.data[i]);
            }
        ...
        _genRoleItem: function(dataItem, i) {
            var self = this;
            var htmlArr = [];
            htmlArr.push('<div class="team-list-list-item-container clearfix">');
                ...
                htmlArr.push('<span class="col2" id="rolecombox-' + i + '"></span>');
            htmlArr.push('</div>');
            return htmlArr.join('');
        },
        _bindRoleItemEvent: function(dataItem, i) {
            var self = this;
            new RoleCombox("#rolecombox-" + i, RoleCombox.getRolePair(dataItem.role), function(role) {
                self.combox_cb && (self.combox_cb(dataItem, role));
            },function(dataItem, role){return self.combox_selectCallback&&self.combox_selectCallback(dataItem,role)});
        },
        appendRoleItem: function(dataItem) {
            var self = this;
