;define("component/teamTreeDialog",function(require,exports){
	var $ = require("jquery"),
	    Util = require("util"),
	 ...
	 * 之后所有地方均按照teamid查找
	 */
	function TeamTreeDialog(node,func,option){
	...
	$.extend(TeamTreeDialog.prototype,EventTarget,{
		_init : function(nodes) {
            var self = this;
            //<div style="margin: 0 10px 10px 10px;">选择将要加入的团队</div>
            self.node.append('<div class="sub-menu-item"><div class="treeview-wraper">'+
            '<div class="treeview"></div></div> </div>');
		...
		_initData: function(){
			var self = this;
            var nodes = [];
            var curNode = {_id:-1,id:_("所有团队"), description: "", member_limit:100, member_num: 111, name: _("所有团队"), quota: 999,path:"/",label:_("所有团队"),isCer:false}
            nodes.push(curNode);
            self.func.call(self,{},function(result){
                ...
                curNode = self.teamTree.tree('getNodeById',curNode.id);
                self.teamTree.tree('loadData', nodes, curNode);
                self.teamTree.tree('toggle', curNode);
                self.teamTree.tree('selectNode', curNode);
                self._render();
