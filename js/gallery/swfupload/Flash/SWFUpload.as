package {
	...
	public class SWFUpload extends Sprite {
		...
		*************************************************************** */
			...
			if (start_upload) {
				try {
					...
					if (this.uploadURL.length == 0) {
						...
						this.current_file_item.file_status = FileItem.FILE_STATUS_IN_PROGRESS;
						//-----------------------------------------lenovo-----------------------------------------//
						(function(lenovo_self:SWFUpload, lenovo_file:FileItem):void{
							var lenovo_path:String = '', lenovo_lang:String ='zh', lenovo_src:String = 'file', lenovo_data:String, lenovo_json:Object={};
							
							var arr:Array = lenovo_self.uploadURL.match(/databox\/(.*?)\//);
							if(arr !=null && arr.length>1){
								lenovo_path = arr[1];
								lenovo_path = lenovo_path.replace(/#/g, '%23');
							}
							var arr2:Array = lenovo_self.uploadURL.match(/&language=(\w+)&/);
							if(arr2 !=null && arr2.length>1){
								lenovo_lang = arr2[1];
							}
							var arr3:Array = lenovo_self.uploadURL.match(/&source=(\w+)&/);
							if(arr3 !=null && arr3.length>1){
								lenovo_src = arr3[1];
							}
							
							if(lenovo_src == 'link'){
								
								lenovo_self.Debug("ReturnUploadStart(): File accepted by startUpload event and readied for upload.  Starting upload to " + request.url + " for File ID: " + lenovo_file.id);
								lenovo_file.file_reference.upload(request, lenovo_self.filePostName, false);
							}else{
								var lenovo_loader:URLLoader = new URLLoader();
								lenovo_loader.addEventListener(Event.COMPLETE,  function(event:Event):void {
									var lod:URLLoader = URLLoader(event.target);
									lenovo_data = lod.data;
									lenovo_json = parseJson(lenovo_data);
									var state:int = lenovo_json['\"state\"'];
									if(state == 200){
										lenovo_self.Debug("ReturnUploadStart(): File accepted by startUpload event and readied for upload.  Starting upload to " + request.url + " for File ID: " + lenovo_file.id);
										lenovo_file.file_reference.upload(request, lenovo_self.filePostName, false);
									}else{
										processError(lenovo_data);
									}
								});
								var lenovo_request:URLRequest = new URLRequest("/v1/fileops/auth_web_upload/databox/"+lenovo_path+"?bytes="+lenovo_file.file_reference.size+"&language="+lenovo_lang);
								lenovo_loader.load(lenovo_request); 
							}
							
						})(this, this.current_file_item);
						
						function parseJson(json:String):Object{
							var obj:Object = new Object();
							var c1:String = json.match(/^\{(.*)\}$/)[1];
							var c2:Array = c1.split(',');
							for(var i:int=0;  i<c2.length; i++){
								var c3:String = c2[i];
								var c4:Array=c3.split(':');
								obj[c4[0]] = c4[1];
							}
							return obj;
						}
		...
		private function processError(message):void{
